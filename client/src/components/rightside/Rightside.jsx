  ...
  return (
    <div className={classes.container}>
      <div className={classes.wrapper}>
        <h2 className={classes.text}>Connected With : </h2>
        {friends?.length > 0 ? (
          friends?.map((friend) => (
             <Link className={classes.user} to={`/profileDetail/${friend._id}`} key={friend._id}>
              <img src={friend?.profileImg ? `http://localhost:5000/images/${friend?.profileImg}` : woman} className={classes.profileUserImg}/>
